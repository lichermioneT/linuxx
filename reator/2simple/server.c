...
// 2.lfd上树
  ...
  ev.data.fd = lfd;
  ev.events = EPOLLIN; // EPOLLIN只是关系它的读事件
  
  int ret = epoll_ctl(lfd, EPOLL_CTL_ADD, lfd, &ev); // 传指针更加的节约内存的
  if(ret == -1)
  {
    perror("epoll_ctl");
    return -1;
...
// 3.上树
  ...
  printf("accept success, threadtid:%ld", info->tid);
  return NULL;
}
...
{
  printf("完整的请求行 : %s", line);

  // get /xx/xxx/.jpg http/1.1
// 路径和方法的解析
  char method[12]={0};
  char path[1024]={0};
...
// 2.响应头
  ...
  // 
  sprintf(buffer + strlen(buffer), "Connetctio: close\r\n");
