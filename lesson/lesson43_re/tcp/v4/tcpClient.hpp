...
// 1. 创建套接字
    ...
    bool start()
    {
        ...
        server.sin_port = htons(_serverport);                   // 服务端的port。
        server.sin_addr.s_addr = inet_addr(_serverip.c_str());  // 服务端的ip地址。

// 2connect
        if (connect(_sock, (struct sockaddr*)&server, sizeof(server)) != 0) // 客户端需要发起链接。
        {
