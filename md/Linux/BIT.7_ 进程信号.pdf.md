...
![image-20260508104439368](picture/image-20260508104439368.png)
...
![image-20260508182153061](picture/image-20260508182153061.png)

![image-20260508182401911](picture/image-20260508182401911.png)
...
![image-20260509111024752](picture/image-20260509111024752.png)
...
**进程的信号信息在PCB里面，不管是硬件还是软件信号，都只能给让OS来进行修改位图结构。**

**PCB的两张表：pending位图，block位图。  还有一个函数指针数组，用来处理不同的信号。**

**signal:找到函数值数组下标，填充数组。**
...
**1.OS的资源，或者硬件资源。所以用户必须通过系统调用。(vector:扩容是要OS的资源。内存的硬件资源。)**

**2.系统调用：必须要进入内核态，才能够执行系统调用。往往系统调用比较费时间一些的。尽量避免频繁调用 系统调用。**

**3.可见寄存器，不可见寄存器。凡是和当前进程相关，都是上下数据。 寄存器：指向PCB, 页表的起始地址，CR3寄存器登场。**

**4.CR3寄存器登场。**

**5.再次理解进程地址空间：3-4G:内核级页表。内核级页表只存在一份的。内核空间被映射到3-4G了。**

**6.内核态的切换只需要在进程地址空间切换就行了的。**

**7.用户为什么？能够访问内核的接口或者数据呢？  运行级别。  系统调用接口，其实的位置会帮你切换状态。（Int 80）陷入内核。进入内核状态。**





![image-20260509155646855](picture/image-20260509155646855.png)
...
![image-20260509155707055](picture/image-20260509155707055.png)

![image-20251128102335873](./picture/image-20251128102335873.png)

 ![image-20251128102455980](./picture/image-20251128102455980.png)

![image-20260509160209781](picture/image-20260509160209781.png)

**内核态向用户态写数据，进程等待的时候STATUS。 内核态不能执行用户态的方法，谁知道你是什么方法呢？？？**
...
![image-20260509163006427](picture/image-20260509163006427.png)

![image-20260509163136444](picture/image-20260509163136444.png)
...
![image-20260509190743888](picture/image-20260509190743888.png)
...
![image-20260509193758990](picture/image-20260509193758990.png)
...
![image-20260509194221330](picture/image-20260509194221330.png)

**子进程给父进程发送信号，我寄了**

![image-20260509195429177](picture/image-20260509195429177.png)
...
![image-20251201220520884](picture/image-20251201220520884.png)





![image-20251201220717512](picture/image-20251201220717512.png)



 **从谈页表**

**物理内存：分成page(页)， 也需要管理起来的  页框。**
